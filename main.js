...
const uri = "mongodb+srv://Ravindu:AE9r61XmOr6shFoA@cluster0.stczp6p.mongodb.net/?retryWrites=true&w=majority";
  // "mongodb+srv://Ravindu:AE9r61XmOr6shFoA@cluster0.stczp6p.mongodb.net/test";
...
app.post('/email',(req, res)=>{
  ...
  form.parse(req, async (err, fields, files) => {
   ...
   //send to data base
    const database = client.db("PCRGDS");
    const contact_us_collection = database.collection("contact_us");
    const result = contact_us_collection.insertOne(fields);
    ...
    if ( !useremail , !username ) {
      ...
      var mailOptions = {
        ...
        // html: "<p>HTML version of the message   text: ", username:  " username      </p>"
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    const { pcrgFirstName,pcrgNic,pcrgPostAddress, pcrgMobile, pcrgMobileResident, pcrgMobileOffice, pcrgEmail,
      pcrgQulification, pcrgQulification1, pcrgSpeciality, pcrgSLMCRegNo, pcrgFaxNo, pcrgProposedBy, pcrgSecondedBy } = fields;
         ...
         //send to data base
    const database = client.db("PCRGDS");
    const pcrg_collection = database.collection("pcrg");
    const result = pcrg_collection.insertOne(fields);
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    const { pcdgFirstName,pcdgNic,pcdgPostAddress, pcdgMobile, pcdgMobileResident, pcdgMobileOffice, pcdgEmail,
      pcdgQulification, pcdgQulification1, pcdgSpeciality, pcdgSLMCRegNo, pcdgFaxNo, pcdgProposedBy, pcdgSecondedBy } = fields;
               ...
               //send to data base
    const database = client.db("PCRGDS");
    const pcdg_collection = database.collection("pcdg");
    const result = pcdg_collection.insertOne(fields);
...
app.post('/ccebdm',(req, res)=>{
  ...
  form.parse(req, async (err, fields, files) => {
   ...
   //send to data base
    const database = client.db("PCRGDS");
    const ccebdm_collection = database.collection("ccebdm");
    const result = ccebdm_collection.insertOne(fields);
    const result1 = ccebdm_collection.insertOne(fields);
...
console.log('Server running at http://127.0.0.1:'+port+'/');
app.get('/copd', upload.array('attachments'), (req, res) => {
    let attachments = []
    for(let i = 0; i<req.files.length; i++){
      let fileDetails = {
        filename:req.files[i].filename,
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    const { fullName, private, governement, other, otherSpecify,Number, Street, City, MobileNumber, Email,
      ...
      const { attachments} = files;
        ...
        //send to data base
    const database = client.db("PCRGDS");
    const copd_collection = database.collection("copd");
    const result = copd_collection.insertOne(fields);
    const result1 = copd_collection.insertOne(fields);
